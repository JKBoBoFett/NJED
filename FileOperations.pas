...
begin
 ...
 begin
  ...
  i:=FO_Containers.IndexOf(ContName);
  if i<>-1 then cf:=TContainerFile(FO_Containers.Objects[i])
  else
   begin
    if FO_Containers.Count>10 then
     for i:=0 to FO_Containers.Count-1 do
     With FO_Containers.Objects[i] as TContainerFile do
      if not Permanent then begin Free; FO_Containers.Delete(i); break; end;
    cf:=OpenContainer(ContName);
    FO_Containers.AddObject(ContName,cf);
   end;
...
var i:Integer;
begin
 i:=FO_Containers.IndexOf(Path);
  if i<>-1 then result:=TContainerFile(FO_Containers.Objects[i])
  else
   begin
    Result:=OpenContainer(Path);
    FO_Containers.AddObject(Path,Result);
   end;
...
Procedure FreeFO_Containers;
var i:integer;
begin
 for i:=0 to FO_Containers.Count-1 do FO_Containers.Objects[i].Free;
 FO_Containers.Free;
end;
...
Initialization
begin
 FO_Containers:=TStringList.Create;
end;

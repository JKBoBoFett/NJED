...
Function NJEDGetNearestVX(lev:TJKLevel;px,py,pz:double;var rx,ry,rz:double;mdist:double):boolean;
...
Function DotProduct(X1,Y1,Z1,X2,Y2,Z2:double):double;
...
Procedure SectorUpdate(sec:integer);
Function SectorAddSurface(sec:integer):integer;
...
TsurfVerts = REcord
      org : array[0..25] of Tverts;
      new : array[0..25] of Tverts;
      end;
...
var
 surfnumverts,vxnum,i,n,cnt,sec,surfIDX,new_surf:integer;
 scale_factor:double;
 x,y,z,vx,vy,vz,dx,dy,dz: double;
 MVec:Tvector;
 vec:TVector;
 faceverts: Tsurfverts;
  newface: array[0..25] of Tface;
  newVertex:TVertex;
...
begin
 ...
 NJEDCalcSurfCenter(surf,MVec.x,MVec.y,MVec.z);
 ...
 for i := surfnumverts - 1 downto (0) do
   ...
   x:=surf.Vertices.GetItem(i).x;
   y:=surf.Vertices.GetItem(i).y;
   z:=surf.Vertices.GetItem(i).z;

   //get vector from surface center
         vx:=x-Mvec.x; vy:=y-Mvec.y; vz:=z-Mvec.z;

         //scale the vector
         vx:=scale_factor*vx; vy:=scale_factor*vy; vz:=scale_factor*vz;

         //get the new vertex's
         dx:=Mvec.x+1*vx; dy:=Mvec.y+1*vy; dz:=Mvec.z+1*vz;
        ...
        faceverts.new[i].x:=vx;
        faceverts.new[i].y:=vy;
        faceverts.new[i].z:=vz;

        newface[5].index[i]:=SectorAddVertex(JedMain.cur_sc,vx,vy,vz);

        level.sectors[JedMain.Cur_SC].AddVertex(vx,vy,vz);
        JedMain.SectorChanged(surf.sector);
        JedMain.Invalidate;
   end;
   ...
   for i :=  0  to surfnumverts - 1  do
         begin
          new_surf:=SectorAddSurface(JedMain.cur_sc);
           SectorUpdate(JedMain.cur_sc);

                //add new vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.new[i].x,faceverts.new[i].y,faceverts.new[i].z));
                SectorUpdate(JedMain.cur_sc);
                //add org vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.org[i].x,faceverts.org[i].y,faceverts.org[i].z));
                SectorUpdate(JedMain.cur_sc);

                if surfnumverts-1 <> i then
                begin
                 //add org vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.org[i+1].x,faceverts.org[i+1].y,faceverts.org[i+1].z));
                SectorUpdate(JedMain.cur_sc);
                //add new vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.new[i+1].x,faceverts.new[i+1].y,faceverts.new[i+1].z));
                SectorUpdate(JedMain.cur_sc);
                end;
                ...
                begin
                //add org vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.org[0].x,faceverts.org[0].y,faceverts.org[0].z));
                //add new vertex to surface
                SurfaceAddVertex(JedMain.cur_sc,new_surf,SectorFindVertex(JedMain.cur_sc,faceverts.new[0].x,faceverts.new[0].y,faceverts.new[0].z));
                end;

         end;

         if  surfnumverts > 3 then
        begin
         new_surf:=SectorAddSurface(JedMain.cur_sc);
         for i :=  0  to surfnumverts - 1  do
         begin
         SurfaceAddVertex(JedMain.cur_sc,new_surf,newface[5].index[i]);
         end;
        end;
...
Procedure NJEDCalcUVNormalsFrom(surf:TJKSurface; nv1:integer; var un,vn:Tvector);
var nv2,nv3,i,VertRefx:integer;
...
VertRefx:= NJEDGetRefVertex(surf);
{
...
///
 ...
 //SetVec(b,v3.x-v2.x,v3.y-v2.y,v3.z-v2.z);
 {
 du:= DotProduct(a.x, dxNormalU.x, a.y, dxNormalU.y, a.z, dxNormalU.z);
 dv:= DotProduct(b.x, dxNormalV.x, b.y, dxNormalV.y, b.z, dxNormalV.z);
 ...
 du:= DotProduct(v1.x, un.x, v1.y, un.y, v1.z, un.z);
 dv:= DotProduct(v1.x, vn.x, v1.y, vn.y, v1.z, vn.z);

 du:= DotProduct(v2.x, un.x, v2.y, un.y, v2.z, un.z);
 dv:= DotProduct(v2.x, vn.x, v2.y, vn.y, v2.z, vn.z);
...
if (surf.Normal.dz = 0)  then
 ...
 Unormal.dz:=0;
   ...
   x1:=DotProduct(cx, cy, cz, Unormal.dx,Unormal.dy,Unormal.dz);

   x1:=DotProduct(surf.vertices[1].x, surf.vertices[1].y, surf.vertices[1].z, Unormal.dx,Unormal.dy,Unormal.dz);
   x2:=DotProduct(surf.vertices[2].x, surf.vertices[2].y, surf.vertices[2].z, Unormal.dx,Unormal.dy,Unormal.dz);

   if x1=x2 then
   begin
   x1:=DotProduct(surf.vertices[0].x, surf.vertices[0].y, surf.vertices[0].z, Unormal.dx,Unormal.dy,Unormal.dz);
   x2:=DotProduct(surf.vertices[1].x, surf.vertices[1].y, surf.vertices[1].z, Unormal.dx,Unormal.dy,Unormal.dz);
   end;
end;
...
begin
   ...
   for i:=0 to surf.Vertices.Count-1 do
 begin
   vx:= surf.Vertices.GetItem(i);
   xsum:=xsum+vx.x;
   ysum:=ysum+vx.y;
   zsum:=zsum+vx.z;
 end;
  cx:=xsum/surf.Vertices.Count;

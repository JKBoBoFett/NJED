...
{          except
  ...
  if ttx<>nil then
      begin    //NJED hack to support textures larger then 256*256 (max res with hardware render)
         if (u/ttx.width > 256) or (ttx.height > 256) then
           begin
           vd.u:=u/ttx.width*ttx.width/256;
           vd.v:=v/ttx.height*ttx.height/256;
           end
          else
            begin
            vd.u:=u/ttx.width;

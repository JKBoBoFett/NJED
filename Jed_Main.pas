...
uses
  ...
  FileCtrl, u_undo,JED_COM,tbar_tools, u_3dos, ImgList,dglOpenGL,OBJ_export;
...
type
  ...
  public
    ...
    LastX,LastY:Integer;
    last_x,last_y,last_z:double;
    last_sx,last_sy,last_sz:double;
    {Rect select vars}
...
procedure TJedMain.CreateParams(var Params: TCreateParams);
 ...
 begin
  ...
  with Params do begin
      Style:=Style or WS_CLIPCHILDREN or WS_CLIPSIBLINGS or WS_EX_COMPOSITED;
 ...
 //    Params.ExStyle := Params.ExStyle or WS_CLIPSIBLINGS or WS_CLIPCHILDREN;
...
try
 ...
 With clMapGeoBack do Renderer.SetColor(CL_Back,r,g,b);   // clMapGeoBack:TJedColor=(r:127;g:127;b:127);
 ...
 With clMapGeo do Renderer.SetColor(CL_Front,r,g,b);   // clMapGeo:TJedColor=(r:255;g:255;b:255);
...
{Draw Selection}
 ...
 case Map_mode of
  MM_SC: begin
          ...
          With clMapSelBack do Renderer.SetColor(CL_Back,r,g,b)    //clMapSelBack:TJedColor=(r:127;g:0;b:0);
          else With clSelMSelBack do Renderer.SetColor(CL_Back,r,g,b);   //clSelMselBack:TJedColor=(r:127;g:63;b:0);

          Renderer.SetCulling(r_CULLBACK);
        Renderer.DrawPolygons(sec.surfaces);

         if scsel.FindSC(Cur_SC)=-1 then
          With clMapSel do Renderer.SetColor(CL_Front,r,g,b)   //clMapSel:TJedColor=(r:255;g:0;b:0);
          else With clSelMSel do Renderer.SetColor(CL_Front,r,g,b); // clSelMsel:TJedColor=(r:255;g:127;b:0);


          Renderer.SetCulling(r_CULLFRONT);
         Renderer.DrawPolygons(sec.surfaces);     //makes red
  ...
  MM_SF: begin
        ...
        //  With level.Sectors[Cur_SC].surfaces[Cur_SF].Vertices[0] do Renderer.DrawVertex(x,y,z);
         ...
         With level.Sectors[Cur_SC].surfaces[Cur_SF].Vertices[0] do Renderer.DrawVertex(x,y,z);
          ...
          //NJED surface vertex selection
          if SurfaceVertexLightMode=true then
           begin
           With level.Sectors[Cur_SC].surfaces[Cur_SF].Vertices[SelectedSurfaceVertex] do
             begin
              Renderer.DrawVertex(x,y,z);
              VertexLight:= level.Sectors[JedMain.Cur_SC].surfaces[JedMain.Cur_SF].TXVertices[SelectedSurfaceVertex].Intensity;
              if VertexLight > 1 then VertexLight:=1.0;
      ...
      //         Renderer.DrawVertex(txv.u,txv.v,0);
      //     end;
  ...
  //PFPS.Caption:=   FloatToStr(round(FPS));
   if WireFrameAPI = WF_Software then   PFPS.Caption:= 'Software';

   if WireFrameAPI = WF_OpenGL then   PFPS.Caption:= 'OpenGL';
...
{  VK_INSERT: Case map_mode of
 ...
 case key of
  ...
  187,VK_ADD: begin Renderer.scale:=Renderer.scale/1.5; end;
  189,VK_SUBTRACT: begin Renderer.scale:=Renderer.scale*1.5; end;
  ...
  {`}  192: begin  uvedit.Show end;     //NJED
...
begin
 ...
 if WireFrameAPI = WF_OpenGL then
    begin
    firstpass:=true;
 ...
 begin
  ...
  if snaptoVertex then GetNearestVX(Level,x,y,z,x,y,z,9000)
...
begin
 ...
 if WireFrameAPI = WF_Software then
   begin
   ...
   Canvas.MoveTo(CleaveOrgXY.X,CleaveOrgXY.Y);
    ...
    if snaptoVertex then GetNearestVX(Level,fx,fy,fz,fx,fy,fz,9000)   //njed changed from 0.2
   ...
   if WireFrameAPI = WF_OpenGL then
    begin
    Renderer.UnProjectPoint(X,Y,0,px,py,pz);
    if not Renderer.GetGridAt(CleaveOrgXY.X,CleaveOrgXY.Y,lX,lY,lZ) then exit;
    if not Renderer.GetGridAt(X,Y,fX,fY,fZ) then exit;
    if snaptoVertex then GetNearestVX(Level,fx,fy,fz,fx,fy,fz,9000)
    else if SnapToGrid then Renderer.GetNearestGrid(fx,fy,fz,fx,fy,fz);
    // PMSG.Caption:=   FloatToStr(px)+' '+FloatToStr(py)+' '+FloatToStr(pz);

    glColor4f(1.0, 0.0, 0.0, 0.5);

   if not firstpass then //redraw last line to erase
    begin
    glBegin(GL_LINES);
    glVertex3f(lx, ly, lz);
...
try
 if not CleaveStarted then exit;
  CleaveStarted:=false;
    ...
    Redraw;   //njed software line wont clear in xp

 if not Renderer.GetGridAt(X,Y,X2,Y2,Z2) then exit;
 if snaptoVertex then GetNearestVX(Level,x2,y2,z2,x2,y2,z2,9000)
...
if WireFrameAPI = WF_Software then
 ...
 if WireFrameAPI = WF_OpenGL then
    begin
        ...
        glLoadIdentity();
        glColor4f(1.0, 0.0, 0.0, 0.5);
        glRectd(RectOrgX,vp[3]-RectOrgY,RectLastX, vp[3]-RectLastY);
        glRectd(RectOrgX,vp[3]-RectOrgY,x, vp[3]-y);
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    surf.FaceFlags:= face.ftype;  // NJED 20-Jul-2022
...
begin
 ...
 With GetFileOpen do
 begin
  Filter:='All Importable files|*.lev;*.gob;*.asc;*.3do;*.obj|DF files(*.LEV;*.GOB)|*.LEV;*.GOB|3D Studio ASC|*.asc|JK 3DOs|*.3do;*.gob|Wavefront OBJ|*.obj';
 If Execute then
 begin
  ResettingEditor;
...
var l:integer;
begin
 Caption:=Format('NJed - %s',[LevelFile]);
...
try
 ...
 //ClearUndoBuffer;     //NJED     don't clear undos man...
 sms:=GetMSecs;
...
if (Level.Sectors.IndexOf(stitch_sec)<0) or
 ...
 //NJED
 if ssurf.Material<>'' then esurf.SurfFlags:=ssurf.SurfFlags;
 if ssurf.Material<>'' then esurf.FaceFlags:=ssurf.FaceFlags;
...
procedure TJedMain.ExportSectorsasOBJ1Click(Sender: TObject);
begin
ObjExport.ShowModal
end;
